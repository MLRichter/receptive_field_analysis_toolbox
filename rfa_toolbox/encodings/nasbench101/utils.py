...
def convolutional_config_from_string(config_string: str,
                                     pattern=re.compile(r"Conv\dx\d"),
                                     kernel_size_extractor: Callable[[str], int] =
                                     lambda x: int(x.replace("Conv", "").split("x")[0]),
                                     stride_size_extractor: Callable[[str], int] =
                                     lambda x: 1) -> Optional[LayerDefinition]:
